...
class Unet(nn.Module):
    ...
    def forward(self, input, layers=[], out_layer_ids=None, encode_only=False, verbose=False):
        if len(layers) == 0:
            """Standard forward"""
            enc_feats = []
            if out_layer_ids is not None:
                out_feats = []
            feat = input
            for layer_id, layer in enumerate(self.model):
                ...
                if out_layer_ids is not None:
                    if layer_id in out_layer_ids:
                        out_feats.append(feat)
                    if layer_id == out_layer_ids[-1]:
                        return out_feats
